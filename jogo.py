...
class Jogo:
    ...
    def _obter_coordenadas_validas(self, mensagem):
      while True:
        try:
            coord_str = input(f"{self._get_jogador_atual().get_nome()}, {mensagem} (linha coluna, ex: 1 2): ") 
            r, c = map(int, coord_str.split())
            
            if not (1 <= r <= self.tabuleiro.tamanho and 1 <= c <= self.tabuleiro.tamanho):
                print(f"Coordenadas inválidas. Por favor, digite números de 1 a {self.tabuleiro.tamanho}.")

            elif self.tabuleiro.esta_revelada(r - 1, c - 1): 
